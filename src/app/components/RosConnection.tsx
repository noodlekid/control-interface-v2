...
export const RosConnectionContext = createContext<RosConnectionStatus>({} as RosConnectionStatus);

const RosConnection = ( { children }: {children: React.ReactNode} ) => {
    const [connectionStatus, setConnectionStatus] = useState<RosConnectionStatus["connectionStatus"]>({
        url: '',
        isConnecting: false,
        isConnected: false
    });
    ...
    const connect = (url: string, callback: VoidFunction) => {
        ...
        newRos?.on('close', (e) => {
            console.log('Connection dropped on connection', e);
            setConnectionStatus({
                ...connectionStatus,
                isConnected: false,
                isConnecting: false
    ...
    return (
        <RosConnectionContext.Provider value={{ros, connectionStatus, connect, disconnect }}>
            { children }
